...
const ExploreMixin = (Base) => {
    return class ExploreMixin extends Base {
        ...
        async getMoodCategories() {
            ...
            const naved = (0, utils_1.nav)(response, [...parsers_1.SINGLE_COLUMN_TAB, ...parsers_1.SECTION_LIST]);
        ...
        async getMoodPlaylists(params) {
            ...
            for (const section of (0, utils_1.nav)(response, [
                ...
                if ('gridRenderer' in section) {
                    ...
                    results = (0, utils_1.nav)(section, parsers_1.GRID_ITEMS);
                }
                else if ('musicCarouselShelfRenderer' in section) {
                    // ditto
                    results = (0, utils_1.nav)(section, parsers_1.CAROUSEL_CONTENTS);
                }
                else if ('musicImmersiveCarouselShelfRenderer' in section) {
                    // ditto
                    results = (0, utils_1.nav)(section, [
        ...
        async getCharts(country = 'ZZ') {
            ...
            charts['countries']['options'] = (0, utils_1.nav)(response, parsers_1.FRAMEWORK_MUTATIONS)
                .map((m) => (0, utils_1.nav)(m, ['payload', 'musicFormBooleanChoice', 'opaqueToken'], null))
                .filter((x) => x);
            ...
            const parseChart = (i, parseFunc, key) => {
                return (0, browsing_1.parseContentList)((0, utils_1.nav)(results[i + (hasSongs ? 1 : 0)], parsers_1.CAROUSEL_CONTENTS, null), parseFunc, key).filter((x) => x);
            };
            for (const [i, c] of chartsCategories.entries()) {
                charts[c] = {
                    //@ts-expect-error, we'll set the items later...
                    playlist: (0, utils_1.nav)(results[1 + i], [...parsers_1.CAROUSEL, ...parsers_1.CAROUSEL_TITLE, ...parsers_1.NAVIGATION_BROWSE_ID], null),
                };
            }
            if (hasSongs) {
                charts['songs'] = {
