...
function parseArtists(results, uploaded = false) {
    ...
    for (const result of results) {
        ...
        const artist = {};
        artist['browseId'] = (0, utils_1.nav)(data, _1.NAVIGATION_BROWSE_ID);
        artist['artist'] = (0, utils_1.getItemText)(data, 0);
        (0, utils_1.parseMenuPlaylists)(data, artist);
        if (uploaded) {
            artist['songs'] = (0, utils_1.getItemText)(data, 1)?.split(' ')[0];
        }
        else {
        ...
        artist['thumbnails'] = (0, utils_1.nav)(data, _1.THUMBNAILS, null);
        artists.push(artist);
    }
    return artists;
}
...
async function parseLibraryAlbums(response, requestFunc, limit) {
    let results = (0, utils_1.findObjectByKey)((0, utils_1.nav)(response, [..._1.SINGLE_COLUMN_TAB, ..._1.SECTION_LIST]), 'itemSectionRenderer');
    results = (0, utils_1.nav)(results, _1.ITEM_SECTION);
    if (!results['gridRenderer']) {
        return [];
    }
    results = (0, utils_1.nav)(results, _1.GRID);
    let albums = parseAlbums(results['items']);
    if (results['continuations']) {
        const parseFunc = (contents) => parseAlbums(contents);
        albums = [
            ...albums,
            ...(await (0, utils_1.getContinuations)(results, 'gridContinuation', limit - albums.length, requestFunc, parseFunc)),
        ];
    }
    return albums;
}
...
function parseAlbums(results) {
    ...
    for (const result of results) {
        ...
        const album = {};
        album['browseId'] = (0, utils_1.nav)(data, [..._1.TITLE, ..._1.NAVIGATION_BROWSE_ID]);
        album['title'] = (0, utils_1.nav)(data, _1.TITLE_TEXT);
        album['thumbnails'] = (0, utils_1.nav)(data, _1.THUMBNAIL_RENDERER);
        if ('runs' in data['subtitle']) {
            const runCount = data['subtitle']['runs'].length;
            let hasArtists = false;
            if (runCount == 1) {
                album['year'] = (0, utils_1.nav)(data, _1.SUBTITLE);
            }
            else {
                album['type'] = (0, utils_1.nav)(data, _1.SUBTITLE);
            }
            if (runCount == 3) {
                if ((0, pyLibraryMock_1.isDigit)((0, utils_1.nav)(data, _1.SUBTITLE2))) {
                    album['year'] = (0, utils_1.nav)(data, _1.SUBTITLE2);
                }
                else {
                    hasArtists = true;
                }
                ...
                album['year'] = (0, utils_1.nav)(data, _1.SUBTITLE3);
                hasArtists = true;
            }
            if (hasArtists) {
                ...
                album['artists'].push({
                    name: subtitle['text'],
                    id: (0, utils_1.nav)(subtitle, _1.NAVIGATION_BROWSE_ID, null),
                });
            }
        }
        albums.push(album);
...
async function parseLibraryArtists(response, requestFunc, limit) {
    ...
    if (results['continuations']) {
        ...
        artists = [
            ...artists,
            ...(await (0, utils_1.getContinuations)(results, 'musicShelfContinuation', limit - artists.length, requestFunc, parseFunc)),
        ];
    }
    return artists;
}
